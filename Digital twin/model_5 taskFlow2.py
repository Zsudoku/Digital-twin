#coding:utf-8
'''
Date: 2022-06-07 16:21:08
LastEditors: ZSudoku
LastEditTime: 2022-06-13 21:04:14
FilePath: \Digita-twin\Digital twin\model_5 taskFlow2.py
立库模块，主要计算堆垛机的任务
...
def GetS_H(LisDdjCode):
    ...
    for i in range(len(LisDdjCode)):
        for j in range(len(LisDdjCode[i])):
            #if(CALCjudgeType(LisDdjCode[i][j]) == 'S'):
            if(CargoNow[LisDdjCode[i][j] - 1]['s1'] == 0 and CargoNow[LisDdjCode[i][j] - 1]['s2'] == 1):
                x += 1
